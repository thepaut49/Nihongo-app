...
public interface CounterRepository extends JpaRepository<Counter, Long> {
	...
	@Query("SELECT v FROM Counter v WHERE (:kanjis is null or v.kanjis LIKE  LOWER(concat('%', concat(:kanjis, '%')))) "
			+ " and (:pronunciation is null or EXISTS(SELECT vp from CounterPronunciation vp WHERE v.id = vp.counter.id AND vp.pronunciation LIKE LOWER(concat('%', concat(:pronunciation, '%')))))"
