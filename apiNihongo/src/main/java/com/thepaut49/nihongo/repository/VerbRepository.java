...
public interface VerbRepository extends JpaRepository<Verb, Integer> {
	...
	@Query("SELECT v FROM Verb v WHERE (:neutralForm is null or v.neutralForm LIKE  LOWER(concat('%', concat(:neutralForm, '%')))) "
			+ " and (:pronunciation is null or EXISTS(SELECT vp from VerbPronunciation vp WHERE v.id = vp.verbId AND vp.pronunciation LIKE LOWER(concat('%', concat(:pronunciation, '%')))))"
			+ " and (:meaning is null or EXISTS(SELECT vm from VerbMeaning vm WHERE v.id = vm.verbId AND vm.meaning LIKE LOWER(concat('%', concat(:meaning, '%')))))"
