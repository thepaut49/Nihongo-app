package com.thepaut49.nihongo.repository.iadjective;
...
public interface IAdjectiveRepository extends JpaRepository<IAdjective, Long> {
	...
	@Query("SELECT v FROM IAdjective v WHERE (:kanjis is null or v.kanjis LIKE  LOWER(concat('%', concat(:kanjis, '%')))) "
			+ " and (:pronunciation is null or EXISTS(SELECT vp from IAdjectivePronunciation vp WHERE v.id = vp.iAdjective.id AND vp.pronunciation LIKE LOWER(concat('%', concat(:pronunciation, '%')))))"
			+ " and (:meaning is null or EXISTS(SELECT vm from IAdjectiveMeaning vm WHERE v.id = vm.iAdjective.id AND vm.meaning LIKE LOWER(concat('%', concat(:meaning, '%')))))")
