...
foreach ($file in $controllersJava)
{
    ...
    #((Get-Content -Path ..\apiNihongo\src\main\java\com\thepaut49\nihongo\controller\$file -Raw) -replace '192.168.1.10','HOST_NAME') | Set-Content -Path ..\apiNihongo\src\main\java\com\thepaut49\nihongo\controller\$file
}
...
((Get-Content -Path '..\front-end-nihongo-redux\keycloak.json' -Raw) -replace 'HOST_NAME',$config.host_name) | Set-Content -Path '..\front-end-nihongo-redux\keycloak.json'
